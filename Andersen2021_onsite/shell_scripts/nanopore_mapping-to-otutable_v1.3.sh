...
# Settings
FASTQFOLDER=gup315_demulti_concat;
...
DATABASE_FASTA=/srv/MA/users/mha/databases/midas37/midas37_notax.fa; # Path to fasta database for mapping
DATABASE_TAX=/srv/MA/users/mha/databases/midas37/tax_complete_qiime.txt # Path to taxonomy of database set appropriate error handling
######
...
checkFolder 2020-09-28_temp
#checkFolder output
####
mkdir -p 2020-09-28_temp/mapped/
mkdir -p 2020-09-28_temp/filtered/
...
#done
#listoffastq=$(find $FASTQFOLDER/barcode*_combined/ -name '*.fastq' -printf '%h\n' | sort -u)
for f in $FASTQFOLDER/*.fastq; do
barcodename=$(basename "$f" .fastq)
    (
    mkdir -p $FASTQFOLDER/concatenated/
	  echo "Trimming $barcodename"
      #cat $dirpath/*.fastq | \
      sed 's/\(runid.*start_\)//' $f | \
...
NAME=$(basename "$f" .fastq)
if [ -s 2020-09-28_temp/$NAME.idmapped.txt ]; then echo "temp/$NAME.idmapped.txt has already been generated";  
else
...
minimap2 -ax map-ont \
  ...
  $DATABASE_FASTA \
  -K20M $f > \
  2020-09-28_temp/mapped/$NAME.sam
module load SAMtools/1.10-foss-2018a
samtools view -F 256 \
  -F 4 \
  -F 2048 2020-09-28_temp/mapped/$NAME.sam \
  -o 2020-09-28_temp/mapped/$NAME"_nodupes.sam"
# Create mapping overview
echoWithHeader "Creating mapping overview"
sed '/^@/ d' 2020-09-28_temp/mapped/$NAME"_nodupes.sam" | \
	awk '{
    for(i=1;i<=NF;i++){
   ...
   sed 's/time=/\t/' | sed 's/Zbarcode=/\t/' > 2020-09-28_temp/$NAME.idmapped.txt
   if  [ -s 2020-09-28_temp/$NAME.idmapped.txt ]
   then
     echo "Mapped $NAME succesfully"
   else
     echo "Mapping for file $NAME did not produce results. Deleting"
     rm -f 2020-09-28_temp/$NAME.idmapped.txt
   fi
fi
done
...
files<-list.files(path = "2020-09-28_temp/", pattern = ".idmapped.txt")
for (file in files){
  mapping <- fread(
    paste0("2020-09-28_temp/",file),
    header = FALSE,
    #select = c(3),
    #colClasses = "character",
	col.names = c("readID", "read_time", "add_info")
  ) %>% separate(add_info, c("barcode", "SAMflag", "OTU", "Qlen", "alnlen", "MapID", "NMtag", "alnscore", "MinimapID"), " ") %>% mutate(SeqID=sub("_combined.idmapped.txt","",file))
...
if (question == TRUE) {
  fwrite(mappings_s, "2020-09-28_temp/mappings_midas37.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
#write out
fwrite(BIOMotutable, paste0("2020-09-28_temp/otutable_", args[[3]], ".txt"), sep = "\t", col.names = TRUE, na = "NA", quote = FALSE)
